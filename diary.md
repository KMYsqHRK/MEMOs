...
東京大学大学院工学系研究科　
糸井・小山研究室　御中
はじめまして。突然のご連絡、大変失礼いたします。
先日、東京大学の五月祭にて貴研究室の展示「ロボットと協働する耐災害スマートシティ」を拝見し、大変興味深く聞かせていただきました。
今回、その時の感謝と、思い付いたアイデアをお伝えしたく、大変失礼かと存じますが、メールさせていただいた次第です。
（宛先が間違いでしたら、大変申し訳ございませんが、ご担当に転送していただければありがたいです）
展示でお話をさせていただいた際には、
○災害時におけるロボット制御のための通信基盤の確保
○がれきなどが存在する中でのロボットの移動と形状の工夫
○停電時を想定した動力源の選定（化石燃料の利用可能性）
○人間を補助するパワースーツの導入可能性
などを中心に質問させていただき、丁寧にご説明をしていただき、大変勉強になりました。
帰宅後、展示内容を思い返す中で、素人ながら一つアイデアを思いつきましたので、ご参考までに共有させていただければと思い、メールをさせていただいた次第です。
【アイデアの背景】
○災害時には通信インフラ破壊の可能性があり、自律的なロボット運用の障害となる
（破壊されなかったとしても、破壊される可能性を考慮する必要がある）
○ロボットが自律的に動くためには、周囲状況の把握が不可欠
（ロボット単体では、ロボットから見える範囲の情報しか把握できない）
○ロボットステーションへの経路が寸断される可能性がある
（エネルギー供給等のためステーションへの帰還が前提）
【具体的な提案】
普段は物流に活用される「民生用ドローン」を用い、災害時には以下の機能を担わせることが考えられるのではないでしょうか
○通信の中継・確保
○センサー・カメラによる上空からの情報収集
○地上ロボットに周囲状況をリアルタイムで提供
【期待される効果】
○通信基盤が破壊された場合でも、上空からの通信ネットワークの確保が可能
○周囲状況の把握により、ロボットの行動や優先順位の最適化が期待される
○ドローンによる俯瞰マップの作成と共有により、ステーションへのルート再設定が可能
なお、災害時の通信の確保だけであれば、衛星通信ネットワークを活用することで、地上の通信インフラが破壊されても運用が可能ですが（ウクライナ戦争ではそのような運用がされているようですね）、周囲の状況の俯瞰的なデータ把握やその情報の共有も必要であり、そのためにはドローン活用が有効ではないかと考えました。
さらに、展示で伺った「平時は民生用途に活用し、災害時には災害対応が可能なロボットを目指す」というお話に強く共感しました。
この考え方に基づき、民生用ドローンや地上移動ロボットに災害対応機能（センサー、カメラ、マイクなど）を搭載すれば、平時の有効活用と災害時の即応性を両立できると考えます。
また、これにより民生用のロボットに公的支援の導入もしやすくなるため、民生分野でのロボット導入に弾みがつく可能性があるのではないかと思いました。
以上、まったくの素人の発想にすぎませんが、貴研究室の先進的な取り組みに感銘を受け、どうしてもお伝えしたくご連絡差し上げた次第です。
ご多用のところ恐縮ではございますが、ご笑覧いただければ幸いです。
今後の貴研究室のさらなるご発展とご活躍を心よりお祈り申し上げます。
敬具
~~~

## 250528
久しぶりにまともに研究している。自分の研究についていくつかの気づきを得た
- まず、何らかのクライテリアを決定し、それから滑動モードの分析を行う必要がある
- クライテリアについては、サージ荷重がわかればある程度推測できそうなこと
- 摩擦係数などの初期不正によって変化しうるので、乱数要素を多少入れる必要があること
- 現在の検討はあくまで模型上の話なので、最終的にはフルード数を用いた無次元化をしてそれによる滑動モード分類をする必要があること。
などである。

## 250529

粒子で物体を表現するのか、ポリゴンで物体を表現するのか

- 壁の位置をどうするのか
- 圧力のサンプリング法
- 計算効率

これらが問題になる。それぞれの結果を比較することが判断材料になる。

- 噴流、性水圧のパターンを作成
- ポリゴン壁について、壁表面、粒子半分離す、粒子一個離す、のサンパターン、単純平均。MPS重み付け平均の６パターンについて圧力の分布結果を知る
- 粒子壁について、粒子の中心で取得

## 250530

ros2のspin()関数について、
- 登録したコールバック関数を指定したFPSで実行する
- nodeオブジェクトを__init__()で作成したときに決定される

## 250531

LLMについて、情報を大量に溜め込む彼らを使う利点として、多分野の知見をしっかり混ぜ込めることにあるのではないかと思う。松本さんの発表でも、分野間での知識のピンチポイントが生まれてしまうことが示されていた。学習地震工学という枠組みは全ての知見を混ぜ合わせた学習モデルを作成し素早い意志決定を可能にする、いわば地震GPTの作成を目指す取り組みなのだろう。

知見を混ぜ混むことで、工学的諸問題への解決方法と経済的諸問題への解決方法をあわせた問題解決ができたり、可能性は広がっているように見える。

もはや我々に必要なのは、審美眼、哲学というものなのではないか？人間が積み重ねた知識に新たな知識を足す取り組みはもはや一部の人にしかできない。どのように堀当て、生かすのか、それに多くがかかっているように思う。

アクティブラーニングという取り組みは、何かの主体になることで問題意識を実感し、それに対する解決策はこれですよ、と教えることで定着を促すものだと思うんだけど、この設計って難しいよね。